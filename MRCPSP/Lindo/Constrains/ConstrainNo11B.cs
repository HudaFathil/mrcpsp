                ...
                foreach (Step s1 in prob.StepsInProduct[f])
                {
                    ...
                    foreach (Step s2 in prob.StepsInProduct[f])
                    {
                        for (int j = 0; j < prob.JobsInProduct[f].Count; j++)
                        {
                            ...
                            Logger.Instance.debug("Constrain No " + LindoContainer.Instance.ConstrainsCounter + ") T" + jobs[j].Id + "," + f.Id + "," + s2.Id + LindoContainer.TjfiType + " - T" + jobs[j].Id + "," + f.Id + "," + s1.Id + LindoContainer.TjfiType + " <= " + mode.getTotalProcessTime() + " + 9999" + cons.MaxQueueTime);
